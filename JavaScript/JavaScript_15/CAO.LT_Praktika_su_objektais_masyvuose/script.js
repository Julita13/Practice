...
let maleNumber = data.reduce((a,b) => b.gender === "Male" ? a + 1 : a, 0);
...
const males = data.reduce((acc, item) => {
    if (item.gender === "Male") acc++;
    return acc;
  }, 0)
  console.log(males);
...
const newCars = data.filter(v => v.car_year > 2000).map(x => x.id);
console.log(newCars);
...
const ids = data.reduce((acc, item) => {
    ...
    return acc;
  }, []) // acc iškart sukuria masyvą tuščią. jei būtų {} sukurtų objektą
  console.log(ids)
...
const littleSizes = data
  .filter(v => v.shirt_size === "XS" || v.shirt_size === "S").map(x => x.first_name).sort();
console.log(littleSizes)
...
const marskiniai = data.reduce((acc, item) => {
  if (item.shirt_size === "XS" || item.shirt_size === "S") {
    acc.push(item.first_name);
  }
  return acc;
}, []) // acc iškart sukuria masyvą tuščią. jei būtų {} sukurtų objektą
.sort() 
console.log(marskiniai)
...
// or EDITOS VERSIJA // BLOGAI PADARYTA?


const masyvastrys = data
  .reduce((acc, item) => {
    if (item.shirt_size === "XS" || item.shirt_size === "S") {
      acc.push(` ${item.id} ${item.first_name} ${item.shirt_size}`);
    }
    return acc;
  }, []) // acc iškart sukuria masyvą tuščią. jei būtų {} sukurtų objektą
  .sort()
